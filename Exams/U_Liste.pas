...
(* Constructeur *)
   ...
   // creerListeOrdreDec(n) construit la liste (n, n-1, n-2, ..., 2, 1)
   function creerListeOrdreDec(n : CARDINAL) : LISTE;
   
   // creerListeOrdreInc(n) construit la liste (1, 2, 3, ..., n-1, n)
   function creerListeOrdreInc(n : CARDINAL) : LISTE;
...
(* Constructeur *)
   ...
   begin
      if n = 0 then
         creerListeOrdreDec := LISTEVIDE
      else
         creerListeOrdreDec := ajouteEnTete(n,creerListeOrdreDec(n-1));
   end {creerlisteOrdreDec};
   
   // creerListeOrdreInc(n) construit la liste (1, 2, 3, ..., n-1, n)
   function creerListeOrdreInc(n : CARDINAL) : LISTE;
   begin
       creerListeOrdreInc := miroir(creerListeOrdreDec(n));
   end {creerListeOrdreInc};
   ...
   begin
      if estListeVide(L) then begin
         ...
         if  inferieur(e,tete(L))  then begin
...
(* Prédicat *)
   ...
   begin
     ...
     if estListeVide(L) then
      ...
      if ((not(estListeVide(reste(L)))) and (not(inferieurOuEgal(tete(L),tete(reste(L)))))) then
          // L n'est pas triée  
          estTriee:= FALSE 
      else
          // recursivité :
   ...
   // contient(l,ls) = VRAI si et seulement si
   // sl est inclut dans l, autrement dit,
   // sl est une sous-liste de l.
   function contient(l:Liste; sl:Liste):Boolean;
   var
     cl,cls: Liste;
     trouve,cont: Boolean;
